...
public class Admin implements User {
    ...
    public void addClient( int id, String name, String password, ArrayList<User> user) {
    ...
    public void delClient(int id, ArrayList<User> user) {
    ...
    public void createModer(ArrayList<User> users, int id) {
        for (int i = 0; i < users.size(); i++) {
            if (users.get(i).getId() == id) {
                Moder moder = new Moder();
                moder.setId(id);
                moder.setName("moderator " + users.get(i).getName());
                moder.setPassword(users.get(i).getPassword());
